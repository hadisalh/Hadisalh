...
const AccordionItem: React.FC<{ title: string; children: React.ReactNode; isOpen: boolean; onClick: () => void; }> = ({ title, children, isOpen, onClick }) => {
  return (
    <div className="border-b border-slate-700 last:border-b-0">
      <button
        onClick={onClick}
        className={`w-full flex justify-between items-center text-right p-4 focus:outline-none transition-colors ${isOpen ? 'bg-slate-700/30' : 'hover:bg-slate-700/50'}`}
        aria-expanded={isOpen}
      >
        <h2 className={`text-xl font-bold transition-colors ${isOpen ? 'text-amber-400' : 'text-slate-100'}`}>{title}</h2>
...
export const AdhkarView: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 sticky top-0 z-20 bg-slate-900/95 backdrop-blur-sm py-2 -mx-2 px-2 border-b border-slate-800/50">
            <div className="flex items-center gap-3">
                <ShieldCheck className="w-8 h-8 text-amber-400" />
                <h1 className="text-3xl font-bold text-amber-400">حصن المسلم</h1>
            </div>
            <button 
                onClick={resetAllAdhkar}
                className="flex items-center justify-center gap-2 bg-slate-700/80 hover:bg-amber-500 hover:text-slate-900 text-slate-200 px-4 py-2 rounded-lg text-sm font-bold transition-colors shadow-md border border-slate-600 hover:border-amber-400"
            >
                <RotateCcw className="w-4 h-4" />
                بدء يوم جديد (تصفير الأذكار)
            </button>
        </div>
        ...
        <div className="overflow-y-auto flex-grow space-y-8 pb-20">
            ...
            <div className="space-y-4">
                <div className="px-2">
                    <h3 className="text-2xl font-bold text-slate-100 mb-1">الأذكار المقيدة</h3>
                    <p className="text-slate-400 text-sm">الأذكار المرتبطة بوقت أو عدد معين. يتم حفظ تقدمك تلقائياً.</p>
                </div>
                
                <div className="rounded-xl border border-slate-700 bg-slate-800 overflow-hidden shadow-xl animate-fade-in">
                {adhkarData.map((category, index) => (
                    ...
                    <div className="space-y-6">
                        ...
                        <div key={itemIndex} className="relative p-5 bg-slate-900/40 rounded-xl border border-slate-700/50 hover:border-amber-500/30 transition-all">
                             {/* Right Accent Line */}
                             <div className="absolute right-0 top-6 bottom-6 w-1 bg-amber-500/50 rounded-l-full"></div>
                             
                            <div className="pr-3">
                                <p className="text-slate-100 leading-loose text-lg mb-5 font-serif text-justify pl-1">{item.text}</p>
                                <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-4 border-t border-slate-700/30">
                                    ...
                                    <div className="flex items-center gap-2 text-slate-400 bg-slate-900/50 px-3 py-1.5 rounded-lg text-xs sm:text-sm border border-slate-700/50 w-full sm:w-auto justify-center sm:justify-start">
