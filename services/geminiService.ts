...
export async function getGuidance(userInput: string): Promise<GuidanceResponse> {
  ...
  while (retries < MAX_RETRIES) {
    try {
      ...
      if (!jsonText) {
          throw new Error("Received an empty response from the AI model. This might be due to content filtering.");
      }
  ...
  // This line should not be reachable if the logic is correct, but TypeScript needs a return path.
